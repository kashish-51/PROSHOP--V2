...
// local storage can only hold strings.that's why we parse it convert into js object
...
const cartSlice = createSlice({
 ...
 reducers: {
    addToCart: (state, action) => { //state is just the current state of cart  and in action there is data inside payload which we can access by action.payload
        ...
        //x is current item in cart and if it matches to item id in payload the it simply means that the item exist in cart
